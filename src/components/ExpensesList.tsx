...
const ExpenseItem: React.FC<ExpenseItemProps> = ({ item, className, first = false, ...props }) => {
  return (
    <div key={item.id} className={twMerge('flex flex-row p-2 rounded-md bg-blue-50 mb-1', className)} {...props}>
      <Label className='w-1/3'>{item.title}</Label>
  ...
  return (
    <div className='border-b px-4' onClick={() => setExpand((preExpand) => !preExpand)}>
      <div className='flex flex-row items-center justify-between  h-14 '>
        <div className='w-1/5 flex flex-row item-center'>
          <div className='rounded-full bg-blue-100 p-2'>
            <DynamicIcon icon={expense.category.icon} className='text-lg' />
          </div>
          <Label className='ml-2 my-auto'>{expense.category.title}</Label>
        </div>
        <div className='w-1/5 flex flex-row flex-wrap justify-center gap-1'>
          <Chips>tag 1</Chips>
        </div>
